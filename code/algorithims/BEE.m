...
%BEE algorithm( in ch  selection and  data sending,  The energy level is taken into consideration, ch selction continue till All nodes are covered )
function  net=BEE(net)
%% ch initial setting
...
%% spacial BEE parameters or setting
...
net.nodes.Chprob(allAlive) = net.nodes.Cprob(allAlive).*Cen.*Cde;%dinamic ch probablity  parameter for each node:in BEE this probablity vector dont use ,but parts of it is used instead.... 
...
% chDist=net.netDist(chIndex,chIndex);
% chDist(chDist==0)=max(chDist(:))+100;
% [val,in]=min(chDist,[],2)
...
if  net.disp
disp(['BEE iteration: ' num2str(net.para.chIt) ' ch number: '  num2str(length(chIndex)) ' alive: ' ...
...
title(titleText,'fontSize', 8);xlabel('BEE algorithm');
