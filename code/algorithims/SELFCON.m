...
%SELFCON algorithm( in ch  selection and  data sending,  The energy level is taken into consideration, ch selction continue till All nodes are covered, aditional operation in regarding to BEE is hierarchy ch selction, to decrese coverage  )
function  net=SELFCON(net)
%% ch initial setting
...
%% spacial SELFCON parameters or setting
...
net.nodes.Chprob(allAlive) = net.nodes.Cprob(allAlive).*Cen.*Cde;%dinamic ch probablity  parameter for each node:in SELFCON this probablity vector dont use ,but parts of it is used instead....
...
if  net.disp
    disp(['SELFCON iteration: ' num2str(net.para.chIt) ' ch number: '  num2str(length(chIndex)) ' alive: ' ...
...
%% ploting operation
if  net.plot
subplot(2,3,6);%current sub plot replaced last subplot 1
...
title(titleText,'fontSize', 8);xlabel('SELFCON algorithm');
