...
%HEAD algorithm( in ch  selection and  data sending, The energy level is taken into consideration, ch selction continue till All nodes are covered )
function  net=HEED(net)
%% ch initial setting
...
%% spacial HEED parameters
Pmin = 10^-2;%minimum amount that chprob can get( decrese number of iteration)
net.nodes.Chprob =max( [net.nodes.Cprob.* (net.nodes.E./net.para.E0);0*net.nodes.Cprob+Pmin]);%dinamic ch probablity  parameter for each node
...
while (allcov==0)
    ...
    %% make tentative ch using uncovered nodes
    rTan=rand(1,length(unCoverNodes));% make a randome vector to select some tentative chs
    tentChs=unCoverNodes(net.nodes.Chprob(unCoverNodes)>rTan/4);% all uncoverd nodes that have lower chprob than randome number  selects themselves as tentative chs
    
    %% making final ch using chProb parameter
    final=tentChs(net.nodes.Chprob(tentChs)==1);% all nodes that get  chProb 1 Sooner, will be final nodes
    net.nodes.type(final)=2;% final nodes become ch nodes
    
    %% update chprob parameter of tentative chs and control operation
    net.nodes.Chprob(tentChs)=min(net.nodes.Chprob(tentChs)*2,1);% the maximum  number that a tentative node can get is one
...
if  net.disp
disp(['HEED iteration: ' num2str(net.para.chIt) ' ch number: '  num2str(length(chIndex)) ' alive: ' ...
...
if  net.plot
subplot(2,3,2 );%current sub plot replaced last subplot 1
